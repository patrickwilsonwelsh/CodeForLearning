...
import org.coderetreat.util.EasyFile;
import org.coderetreat.util.LineEnumerator;
 ...
 * @author Edwin Martin
 */
public class GameOfLifeStandalone extends GameOfLifeApplet {
	...
	public static void main(String args[]) {
		GameOfLifeStandalone gameOfLife = new GameOfLifeStandalone();
		...
		public void drop(DropTargetDropEvent event) {
			...
			if (event.isDataFlavorSupported(urlFlavor)) {
				...
				try {
					...
					File droppedFile = list.get(0); // More than one file
															// -> get only first
															// file
	...
	class GameOfLifeGridIO {
		 ...
		 */
		public GameOfLifeGridIO(CellGrid grid) {
			this.grid = grid;
		}
		 ...
		 *            EasyFile-object
		 * @throws IOException
		 * @see org.coderetreat.util.EasyFile
		 */
		...
		public void setShape(Shape shape) {
			...
			Dimension gridDim = gameOfLifeCanvas.getGridDimension();
		...
		public Shape makeShape(String name, String text) {
			...
			Enumeration<String> oldEnum = new LineEnumerator(text);
			while (oldEnum.hasMoreElements()) {
				String line = (String) oldEnum.nextElement();
		...
		public void saveShape() {
			int colEnd = 0;
			int rowEnd = 0;
			Dimension dim = gameOfLifeCanvas.getGridDimension();
...
class AppletFrame extends Frame {
	 ...
	 */
	private static final long serialVersionUID = 1L;
	private final GameOfLifeApplet applet;
	 ...
	 *            applet to show
	 */
	public AppletFrame(String title, GameOfLifeStandalone applet) {
		super(title);
		this.applet = applet;
	...
	private void showAboutDialog() {
		...
		Point p = getLocation();
		new AboutDialog(this, "About the Game of Life", new String[] { "Version 1.5 - Copyright 1996-2004 Edwin Martin", "http://www.bitstorm.org/gameoflife/", jvmProperties }, "about.jpg",
				p.x + 100, p.y + 60);
	}
	 ...
	 */
	private GameOfLifeStandalone getStandaloneGameOfLife() {
		return (GameOfLifeStandalone) applet;
	}
}

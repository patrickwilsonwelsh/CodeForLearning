...
public class GameOfLifeGrid implements CellGrid {
  ...
  public synchronized void next() {
    ...
    // Reset cells
    oldEnum = currentShape.keys();
    while ( oldEnum.hasMoreElements() ) {
      cell = (Cell) oldEnum.nextElement();
      cell.neighbour = 0;
    }
    // Add neighbours
    ...
    // Bury the dead
    // We are walking through an oldEnum from we are also removing elements. Can be tricky.
    oldEnum = currentShape.keys();
    while ( oldEnum.hasMoreElements() ) {
      cell = (Cell) oldEnum.nextElement();
